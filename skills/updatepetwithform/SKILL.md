...
## When to use
Use this skill when the user intent maps to `updatepetwithform` and the user is asking for actionable data retrieval or an operation that this tool can perform.
...
## Parameter Mapping
1. Read the MCP schema for `updatepetwithform` and list required vs optional parameters.
...
4. Call the `updatepetwithform` MCP tool with the correct parameters.
...
## Worked Example
- User intent: "Run `updatepetwithform` for my latest record and show key fields only."
- Action: Resolve the required identifier, call `updatepetwithform`, extract the top-level summary fields.
