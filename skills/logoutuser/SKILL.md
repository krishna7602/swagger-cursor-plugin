...
## When to use
Use this skill when the user intent maps to `logoutuser` and the user is asking for actionable data retrieval or an operation that this tool can perform.
...
## Parameter Mapping
1. Read the MCP schema for `logoutuser` and list required vs optional parameters.
...
4. Call the `logoutuser` MCP tool with the correct parameters.
...
## Worked Example
- User intent: "Run `logoutuser` for my latest record and show key fields only."
- Action: Resolve the required identifier, call `logoutuser`, extract the top-level summary fields.
